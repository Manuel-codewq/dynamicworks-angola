...
import { BookOpen, Calendar, Filter, Video, TrendingUp, ChevronDown, ChevronUp, Users, Download } from "lucide-react";
...
export default function AdminFormadoresPage() {
  ...
  function exportCSV() {
    const header = ["Formador", "Email", "Data", "O que fez", "Vídeo publicado", "Título do vídeo", "Reacções", "Dúvidas dos membros", "Plano amanhã"];
    const rows = reports.map(r => [
      r.user.name, r.user.email, r.date,
      `"${r.feito.replace(/"/g, '""')}"`,
      r.videoPublicado ? "Sim" : "Não",
      r.videoTitulo ? `"${r.videoTitulo}"` : "",
      r.reacoes ?? "",
    ...
    a.href = url; a.download = `formadores_${new Date().toISOString().slice(0,10)}.csv`;
            ...
            <div key={r.id} style={{ borderBottom: "1px solid #1e2d50", paddingBottom: 14, marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  {/* Avatar */}
                  <div style={{
                    width: 34, height: 34, borderRadius: "50%", background: "#1e2d50",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    color: "#f5a623", fontWeight: 800, fontSize: 14, flexShrink: 0,
                    overflow: "hidden",
                  }}>
                    {r.user.avatar
