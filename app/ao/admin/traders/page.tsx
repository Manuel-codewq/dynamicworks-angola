...
import { TrendingUp, Filter, ChevronDown, ChevronUp, Users, Download, Image as ImageIcon } from "lucide-react";

type Report = {
  id: string; date: string; operacoes: number | null; lucro: number | null;
  paresNegociados: string | null; estrategia: string | null;
  observacoes: string; planoAmanha: string | null; imagemProva: string | null;
  user: { id: string; name: string; email: string; avatar: string | null };
};
type Trader = { id: string; name: string; email: string; avatar: string | null };
...
export default function AdminTradersPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/admin/traders?${p}`).then(r => r.json()).then(d => { setReports(d.reports || []); setTraders(d.traders || []); setLoading(false); });
  }, [status, filterUser, filterDate]);

  function exportCSV() {
    const header = ["Trader", "Email", "Data", "Operações", "Lucro (Kz)", "Pares", "Estratégia", "Observações", "Plano amanhã", "URL Prova"];
    const rows = reports.map(r => [r.user.name, r.user.email, r.date, r.operacoes ?? "", r.lucro ?? "", r.paresNegociados ?? "", r.estrategia ? `"${r.estrategia}"` : "", `"${r.observacoes.replace(/"/g,'""')}"`, r.planoAmanha ?? "", r.imagemProva ?? ""]);
    const csv  = [header, ...rows].map(r => r.join(",")).join("\n");
    const blob = new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a"); a.href = url; a.download = `traders_${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url);
  }
  ...
  const totalLucro = reports.reduce((s, r) => s + (r.lucro || 0), 0);
  const totalOps   = reports.reduce((s, r) => s + (r.operacoes || 0), 0);
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div><div style={{ color: "#fff", fontSize: 22, fontWeight: 800 }}>Relatórios dos Traders</div><div style={{ color: "#94a3b8", fontSize: 13 }}>Actividade da equipa de trading</div></div>
          </div>
          <button onClick={exportCSV} style={{ display: "flex", alignItems: "center", gap: 7, background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.3)", color: "#22c55e", borderRadius: 8, padding: "9px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            { label: "Traders", value: traders.length, color: "#22c55e" },
            { label: "Total operações", value: totalOps, color: "#38bdf8" },
            { label: "Lucro total (Kz)", value: totalLucro.toLocaleString("pt-PT"), color: totalLucro >= 0 ? "#22c55e" : "#ef4444" },
          ].map(s => (
            ...
            <div key={r.id} style={{ borderBottom: "1px solid #1e2d50", paddingBottom: 14, marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }} onClick={() => setExpanded(expanded === r.id ? null : r.id)}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ width: 34, height: 34, borderRadius: "50%", background: "#1e2d50", display: "flex", alignItems: "center", justifyContent: "center", color: "#22c55e", fontWeight: 800, fontSize: 14, flexShrink: 0, overflow: "hidden" }}>
                  ...
                  {r.operacoes != null && <span style={{ color: "#94a3b8", fontSize: 12 }}>{r.operacoes} ops</span>}
