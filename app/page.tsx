...
function Cursor({ done }: { done: boolean }) {
  return (
    <span style={{
      ...
      background: "#f5a623", marginLeft: 2, verticalAlign: "middle",
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    // pequeno delay para a página estar pintada antes de começar a escrever
    const t = setTimeout(() => setHeroReady(true), 200);
    return () => clearTimeout(t);
  }, []);

  // Sequência de typewriter: badge → linha1 → linha2 → subtítulo
  const badge = useTypewriter(
    heroReady ? "Plataforma de Opções Binárias · Angola" : "",
    38, 0
  );
  const line1Start = badge.done ? 0 : 99_999;
  const line1 = useTypewriter(heroReady ? "Negocie no Mercado Global" : "", 50, line1Start);
  const line2Start = line1.done ? 0 : 99_999;
  const line2 = useTypewriter(heroReady ? "em Kwanzas" : "", 60, line2Start);
  const subStart = line2.done ? 0 : 99_999;
  const sub  = useTypewriter(
    heroReady
      ? "A primeira corretora de opções binárias angolana com pagamentos em AOA, preços em tempo real e conta demo gratuita."
      : "",
    22, subStart
  );
    ...
    <div style={{ minHeight: "100vh", background: "#0a0f1e", fontFamily: "system-ui, sans-serif", color: "#fff" }}>
      ...
      <section style={{ maxWidth: 900, margin: "0 auto", padding: "80px 24px 60px", textAlign: "center" }}>
        ...
        <div style={{ minHeight: 32, marginBottom: 24 }}>
          <div style={{ display: "inline-block", background: "rgba(245,166,35,0.12)", border: "1px solid rgba(245,166,35,0.3)", borderRadius: 20, padding: "5px 16px", fontSize: 12, fontWeight: 700, color: "#f5a623", letterSpacing: 1, textTransform: "uppercase" }}>
            {badge.displayed}
            {!badge.done && <Cursor done={false} />}
          </div>
        </div>

        {/* Título com typewriter linha a linha */}
        <h1 style={{ fontSize: "clamp(32px, 6vw, 58px)", fontWeight: 900, lineHeight: 1.1, margin: "0 0 20px", letterSpacing: -1, minHeight: "2.5em" }}>
          <span style={{ display: "block" }}>
            {line1.displayed}
            {!line1.done && badge.done && <Cursor done={false} />}
          </span>
          <span style={{ display: "block", background: "linear-gradient(90deg, #f5a623, #fbbf24)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            {line2.displayed}
            {!line2.done && line1.done && <Cursor done={false} />}
            {line2.done && <Cursor done={true} />}
          </span>
        </h1>

        {/* Subtítulo com typewriter */}
        <p style={{ fontSize: 18, color: "#94a3b8", maxWidth: 560, margin: "0 auto 36px", lineHeight: 1.6, minHeight: "4em" }}>
          {sub.displayed}
          {!sub.done && line2.done && <Cursor done={false} />}
        </p>

        {/* Botões aparecem depois do subtítulo */}
        <div style={{
          display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap",
          transition: "opacity 0.6s ease, transform 0.6s ease",
          opacity: sub.done ? 1 : 0,
          transform: sub.done ? "translateY(0)" : "translateY(16px)",
        }}>
          <button onClick={() => router.push("/register")}
        ...
        <p style={{ color: "#374151", fontSize: 12, marginTop: 14, transition: "opacity 0.6s ease 0.3s", opacity: sub.done ? 1 : 0 }}>
      ...
      <FadeSection>
        <section style={{ maxWidth: 900, margin: "0 auto", padding: "60px 24px 0" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
            {assets.map((a, i) => (
              <FadeSection key={a.label} delay={i * 80}>
                <div style={{ background: "#111827", border: `1px solid ${a.color}30`, borderRadius: 10, padding: "10px 20px", fontSize: 14, fontWeight: 700, color: a.color }}>
                  {a.label}
                </div>
              </FadeSection>
            ))}
            <FadeSection delay={assets.length * 80}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16 }}>
          {features.map((f, i) => (
            <FadeSection key={i} delay={i * 100}>
              <div style={{ background: "#111827", border: "1px solid #1e2d50", borderRadius: 14, padding: "24px 20px", height: "100%" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {steps.map((s, i) => (
              <FadeSection key={i} delay={i * 150}>
                <div style={{ display: "flex", gap: 20, alignItems: "flex-start", textAlign: "left", paddingBottom: i < steps.length - 1 ? 32 : 0, position: "relative" }}>
