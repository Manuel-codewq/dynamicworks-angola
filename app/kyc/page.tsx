...
import { Camera, CreditCard, Loader2, RefreshCcw, ScanFace, ShieldCheck, Sun, User, CheckCircle, RotateCcw } from 'lucide-react';

type KYCView = 'intro' | 'face' | 'bi-intro' | 'bi' | 'review';
...
export default function KYCVerificationPage() {
  ...
  const handleFaceCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (faceStep < 3) {
      ...
      setTimeout(() => faceInputRef.current?.click(), 300);
    } else {
  ...
  const handleBICapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (biStep === 1) {
      ...
      setBiStep(2);
      if (e.target) e.target.value = '';
      setTimeout(() => biInputRef.current?.click(), 300);
    } else {
  ...
  const allFacesDone = kycData.faceFront && kycData.faceRight && kycData.faceLeft;
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .kb::after{content:'';position:fixed;bottom:-40%;right:-20%;width:70%;height:70%;background:radial-gradient(ellipse,rgba(245,166,35,.04) 0%,transparent 70%);pointer-events:none}
    .brand{font-family:'Syne',sans-serif;color:var(--gold);font-weight:800;font-size:18px;letter-spacing:3px;text-transform:uppercase;margin-bottom:32px;text-align:center;opacity:.9}
    .footer{position:fixed;bottom:16px;left:0;width:100%;text-align:center;font-size:11px;color:var(--muted);pointer-events:none;z-index:100;letter-spacing:.5px}
    ...
    .tip{display:flex;align-items:center;gap:10px;font-size:13.5px;color:var(--text);margin-bottom:10px;font-weight:400}
    ...
    .btn-out:hover{border-color:rgba(255,255,255,.3)}
    ...
    .face-preview{position:relative;width:100%;aspect-ratio:1;border-radius:16px;overflow:hidden;margin-bottom:16px;background:var(--card2);border:1px solid var(--border)}
    .face-preview img{width:100%;height:100%;object-fit:cover}
    .face-overlay{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(0,0,0,.6);gap:12px}
    ...
    @keyframes pulse-border{0%,100%{border-color:rgba(245,166,35,.5)}50%{border-color:rgba(245,166,35,1)}}
    ...
    .step-hint{color:rgba(255,255,255,.7);font-size:12px;text-align:center;padding:0 20px}
    ...
    .bi-guide{width:100%;aspect-ratio:16/10;background:var(--card2);border:1px solid var(--border);border-radius:16px;display:flex;align-items:center;justify-content:center;margin-bottom:16px;position:relative;overflow:hidden}
    ...
    .bi-rect span{color:rgba(255,255,255,.5);font-size:12px}
  ...
  return (
    <>
      ...
      <div className="kb">
        ...
        {/* Inputs ocultos — funcionam em qualquer browser móvel */}
        <input
          ref={faceInputRef}
          type="file"
          accept="image/*"
        ...
        {currentView === 'intro' && (
          <div className="view">
            <div className="card">
              ...
              <p className="sub">Vamos confirmar a sua identidade com uma verificação facial e o seu B.I.</p>
              <div className="tips">
        ...
        {currentView === 'face' && (
          <div className="view">
            ...
            <div className="card">
              ...
              <button className="btn btn-gold" onClick={() => faceInputRef.current?.click()}>
                <Camera size={18} /> Tirar Foto — {faceSteps[faceStep - 1].label}
              </button>
            </div>
          </div>
        )}
        ...
        {currentView === 'bi-intro' && (
          <div className="view">
            <div className="card">
              ...
              <p className="sub">Fotografe a frente e o verso do seu B.I. com boa iluminação, sem reflexos.</p>
        ...
        {currentView === 'bi' && (
          <div className="view">
            ...
            <div className="card">
              ...
              <p className="sub">{biStep === 1 ? 'Fotografe a frente do documento' : 'Agora vire e fotografe o verso'}</p>
              ...
              <button className="btn btn-gold" onClick={() => biInputRef.current?.click()}>
                <Camera size={18} /> Tirar Foto — {biStep === 1 ? 'Frente' : 'Verso'}
              </button>
            </div>
          </div>
        )}
        ...
        {currentView === 'review' && (
          <div className="view">
            ...
            <div className="card">
              ...
              <button className="btn btn-out" onClick={restartKYC}>
